...
RegisterCommand("admin", function(source, args, rawCommand)
    if source ~= 0 then
        ...
        TriggerClientEvent('chatMessage', xPlayer.source, _U('your_rank', xPlayer.group))
    end
end, false)
...
RegisterCommand("players", function(source, args, rawCommand)   -- players      show online players | console only
    ...
    if source == 0 then
        ...
        for i=1, #xAll, 1 do
            ...
            print("^4[ ^2ID : ^3"..xPlayer.source.." ^0| ^2Name : ^3"..(xPlayer.name).." ^0 | ^2Group : ^3"..(xPlayer.group).." ^4]^0\n")
            isPlayerOnline = true
        end
        ...
        for i=1, #xAll, 1 do
            ...
            if havePermission(xTarget) then     -- you can exclude some ranks to NOT reciveing reports
                if xPlayer.source ~= xTarget.source then
                    TriggerClientEvent('chatMessage', xTarget.source, _U('report', xPlayer.name, xPlayer.source, message))
                end
            end
        end
        TriggerClientEvent('chatMessage', xPlayer.source, _U('report', xPlayer.name, xPlayer.source, message))
...
RegisterCommand("announce", function(source, args, rawCommand)  -- /announce [MESSAGE]
    if source ~= 0 then
        ...
        if args[1] then
            ...
            if xPlayer then
                if havePermission(xPlayer) then
                    TriggerClientEvent('chatMessage',-1 , _U('admin_announce', xPlayer.name, message))
                end
            end
        else
            ...
            if args[1] then
                ...
                for i=1, #xAll, 1 do
                    ...
                    if havePermission(xTarget) then
                        if xPlayer.source ~= xTarget.source then
                            TriggerClientEvent('chatMessage', xTarget.source, _U('adminchat', xPlayer.name, xPlayer.group, message))
                        end
                    end
                end
                TriggerClientEvent('chatMessage', xTarget.source, _U('adminchat', xPlayer.name, xPlayer.group, message))
                    ...
                    if source == tonumber(args[1]) then
                        ...
                        if xTarget then
                            if havePermission(xTarget) then
                                ...
                                TriggerClientEvent("chatMessage", xTarget.source, _U('adminwarn_to', args[1],xPlayer.name, xPlayer.group))
                            else
